"""Precompute 2D UMAP projection of aligned embeddings across all years.

Concatenates every (year, eligible_word) vector into one matrix and runs a
single UMAP fit so the layout is shared across years. A word's yearly
positions then trace a coherent trajectory through the same 2D space.

Outputs (under web/public/data/):
  space.bin            float32, shape [N_YEARS, N_ELIGIBLE, 2], normalized to [-1, 1]
...
def main() -> None:
    ...
    print(
        f"fitting UMAP (n_components=2, n_neighbors={UMAP_N_NEIGHBORS}, "
        ...
        json.dump(
            {
                ...
                "n_words": n_words,
                "bbox": {
                    "min": coords.reshape(-1, 2).min(axis=0).tolist(),
                    "max": coords.reshape(-1, 2).max(axis=0).tolist(),
                },
            },
            f,
        )
