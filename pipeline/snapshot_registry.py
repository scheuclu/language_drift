YEAR_TO_SNAPSHOTS: dict[int, list[str]] = {
...
def get_snapshots(year: int) -> list[str]:
    if year not in YEAR_TO_SNAPSHOTS:
        raise ValueError(f"No snapshots registered for year {year}")
    return YEAR_TO_SNAPSHOTS[year]
