...
export async function loadManifest(): Promise<Manifest> {
  ...
  const res = await fetch("/data/manifest.json?v=2");
...
export async function loadWord(word: string): Promise<WordData | null> {
  ...
  const res = await fetch(`/data/w/${encodeURIComponent(key)}.json?v=2`);
...
export async function loadGallery(): Promise<DriftGallery> {
  const res = await fetch("/data/drift_gallery.json?v=2");
