...
## Web routes slated for deletion

`/gallery`, `/ternary`, and `/explore` are **hidden from the nav** (`web/components/Nav.tsx`) and are candidates for full removal. The live nav is just **story · space · arith · llm**. `/gallery` still routes into the removed `/explore` constellation view; `/explore` survives only as a deep-link target from `/llm`. Unless one earns its place back, delete the route folders (`web/app/{gallery,ternary,explore}`) and any now-unused components/lib (`Constellation`, `Triangle`, `SearchBar`, `featured.ts`, etc.). Until then they still build and resolve by direct URL.
